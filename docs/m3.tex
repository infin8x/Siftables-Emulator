...
The Siftables Emulator will work as a standalone software package and will not interface with any other software or hardware.  Singularity Software considered the integration of the emulator in a \gls{IDE}, but it was decided that the emulator was best served as an independent tool.  Similarly, the emulator is to serve as a method for developing applications for the cubes without the need for the physical hardware, so it will not interface with any hardware either.
