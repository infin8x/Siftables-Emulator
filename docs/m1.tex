...
Developers of applications for the \gls{Sifteo Cubes}\index{Sifteo} rely on the use of the Cube hardware itself to test programs.  With the release of the Cubes and corresponding \gls{API}\index{API} still pending, many are left without a common interface within which to productively develop programs to deploy on the Cubes. As Sifteo\index{Sifteo} prepares for the release of both the Cubes and the \gls{API}\index{API} for programs, an intuitive interface that will make application development more efficient has become necessary.
...
Sifteo has announced that they will be releasing their \gls{API}\index{API} later this month; as of this printing, the \gls{API}\index{API} has not yet been made public.
...
Two attributes accompany each feature described below: \textbf{Status}, which is a measure of the feature's progress duing the project definition period, and \textbf{Priority}, which indicates the relative importance of each feature.
    \begin{table}[h]
      \begin{tabular}{p{1.5in} | p{1.75in} | p{1in} | p{1in}}
        ...
        The user will be able to place a number (1 to 6) of Cubes on the workspace and manipulate them &
        Approved &
        Critical \\ \hline
