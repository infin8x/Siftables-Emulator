...
Developers of applications for the \gls{Sifteo Cubes}\index{Sifteo} rely on the use of the Cube hardware itself to test programs.  With the release of the Cubes and corresponding \nomenclature{API}{Application Programming Interface}\index{API} still pending, many are left without a common interface within which to productively develop programs to deploy on the Cubes. As Sifteo\index{Sifteo} prepares for the release of both the Cubes and the API\index{API} for programs, an intuitive interface that will make application development more efficient has become necessary.
